...
export async function generateQuestions(config: QuizConfig): Promise<Question[]> {
    ...
    - Bangun Datar & Ruang: JIKA SOAL MEMINTA MENGHITUNG TEHADAP SEBUAH "GAMBAR BANGUN RUANG" ATAU "GAMBAR BANGUN DATAR", Anda WAJIB MENAMPILKAN GAMBAR tersebut menggunakan tag [GEOMETRY:shape_name:{"label_key":"label_value"}]. JANGAN hanya mendeskripsikan ukurannya dalam bentuk teks (misal: "Sebuah balok memiliki ukuran panjang 12 cm..."). Anda WAJIB langsung menyisipkan tag geometri ke dalam teks soal (\`questionText\`), lalu diikuti pertanyaannya. DILARANG menggunakan emoji, unicode, atau \`imageSearchKeyword\` untuk bangun ruang/datar. 
      ...
      Daftar \`shape_name\` yang valid HANYALAH: "triangle", "square", "rectangle", "parallelogram", "kite", "rhombus", "trapezoid", "polygon", "circle", "cube", "cuboid", "cylinder", "cone", "sphere", "pyramid", "prism", "combined_cuboid_pyramid", "combined_cuboid_cube", "combined_cylinder_cone", "combined_rect_triangle", "combined_l_shape", "combined_rect_semicircle", "combined_rect_rect", "combined_rect_semicircle", "combined_rect_rect".
    ...
    - PENTING (SINTAKS MATEMATIKA & LATEX): Jika Anda menyisipkan sintaks LaTeX atau matematika (misalnya rumus, pecahan, atau satuan dengan backslash), Anda WAJIB MENG-ESCAPE KODE BACKSLASH TERSEBUT KARENA INI ADALAH FORMAT JSON! Contoh: Tuliskan \\frac{3}{4} BUKAN \frac{3}{4}. Tuliskan \\text{cm}^3 BUKAN \text{cm}^3. Jika Anda hanya menggunakan satu backslash, JSON parser akan menganggapnya sebagai escape character yang tidak valid dan merusak rumus.
    ...
    - PENTING (AKURASI JAWABAN): Anda WAJIB menghitung kunci jawaban secara matematis step-by-step dan memastikannya 100% akurat. Untuk soal hitungan/logika, gunakan field 'explanation' untuk menjabarkan langkah-langkah penyelesaiannya secara detail SEBELUM mengisi 'correctAnswer' dan 'options' agar Anda tidak salah menebak. Jawaban yang Anda berikan harus sama persis dengan hasil perhitungan Anda. Pastikan jika menggunakan pi (π) pada lingkaran/tabung, Anda menggunakan 22/7 jika radius kelipatan 7, dan 3.14 jika bukan.
