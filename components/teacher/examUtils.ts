...
export const analyzeQuestionTypePerformance = (
  exam: Exam,
  ...
  const typeMap: Record<
    string,
    { totalQuestions: number; totalAttempt: number; correct: number }
  > = {};
  ...
  const checkAnswer = (
    ...
    if (
      ...
      try {
        ...
        return (
          q.trueFalseRows?.every((row, idx) => ansObj[idx] === row.answer) ??
          false
        );
      } catch {
        return false;
      ...
      try {
        const ansObj = JSON.parse(ans);
        return (
          q.matchingPairs?.every((pair, idx) => ansObj[idx] === pair.right) ??
          false
        );
      } catch {
        return false;
  ...
  return Object.entries(typeMap)
    .map(([type, data]) => ({
      ...
      percentage:
        data.totalAttempt > 0
          ? Math.round((data.correct / data.totalAttempt) * 100)
          : 0,
    }))
...
export const analyzeClassPerformance = (
  ...
  return Object.entries(groupMap)
    .map(([key, classResults]) => {
      ...
      const averageScore =
        studentCount > 0
          ? Math.round(scores.reduce((a, b) => a + b, 0) / studentCount)
          : 0;
      const averageTime =
        times.length > 0
      ...
      const passRate =
        studentCount > 0 ? Math.round((passCount / studentCount) * 100) : 0;

      // Analyze question type performance for this class specifically
      const questionTypeStats = analyzeQuestionTypePerformance(
        exam,
        classResults,
      );
...
export const stripPrefix = (str: string) =>
  str.replace(/^[A-E1-5][.)]\s*/i, "").trim();

export const isAnswerMatch = (
  ans: string | undefined | null,
  opt: string | undefined | null,
  type: QuestionType,
): boolean => {
...
export const calculateAggregateStats = (exam: Exam, results: Result[]) => {
  ...
  // 3. Process to Array
  const processMap = (
    map: Record<string, { total: number; correct: number }>,
  ) =>
    Object.entries(map)
      .map(([name, data]) => ({
        name,
...
export const analyzeStudentPerformance = (
  ...
  exam.questions.forEach((q) => {
    ...
    const category =
      q.category && q.category.trim() !== "" ? q.category.trim() : "Umum";
    ...
    if (result.answers[manualGradeKey]) {
      ...
      if (
        ...
        const sSet = new Set(
          parseList(String(studentAns)).map((a) =>
            normalize(a, q.questionType),
          ),
        );
        const cSet = new Set(
  ...
  const stats: CategoryStat[] = Object.entries(statsMap)
    .map(([name, data]) => ({
      name,
      total: data.total,
      correct: data.correct,
  ...
  const weakestCategory =
    stats.length > 0 ? stats[stats.length - 1].name : null;
  const weakestScore =
...
export const refineImageContent = (dataUrl: string): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const checkOriginalInk = (idx: number) =>
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        reject(new Error("Gagal mengonversi PDF."));
      }
    };
...
export const parsePdfAndAutoCrop = async (file: File): Promise<Question[]> => {
        ...
        getDocument: (opts: unknown) => {
          promise: Promise<{
            ...
            getPage: (
              ...
              getViewport: (opts: unknown) => { width: number; height: number };
  ...
  for (let i = 1; i <= numPages; i++) {
    ...
    items.forEach((item) => {
      ...
      if (!currentLine) {
        ...
        const isSameLine =
          verticalDist < Math.max(currentLine.height * 0.8, 15);
        if (isSameLine) {
  ...
  const getCropRect = (anchor: Anchor) => {
    ...
    return {
      ...
      w: MAX_X - MIN_X,
      h: MAX_Y - MIN_Y,
      bottomAnchorFound: foundBottomAnchor,
    };
  };

  const processAnchorCrop = async (
    anchor: Anchor,
    index: number,
  ): Promise<string> => {
    ...
    if (rect.bottomAnchorFound || anchor.type === "OPTION") {
      ...
      if (nextAnchor && nextAnchor.pageIdx === anchor.pageIdx + 1) {
        ...
        i2.src = img2;
        await Promise.all([
          new Promise((r) => (i1.onload = r)),
          new Promise((r) => (i2.onload = r)),
        ]);
        ...
        questions.push({
          ...
          options:
            currentOptions.length > 0
              ? await Promise.all(currentOptions.map((o) => o.promise))
              : undefined,
          correctAnswer:
            currentOptions.length > 0
              ? await currentOptions[0].promise
              : undefined,
          category: "",
          level: "",
        });
    ...
    questions.push({
      ...
      options:
        currentOptions.length > 0
          ...
          : undefined,
      correctAnswer:
        currentOptions.length > 0 ? await currentOptions[0].promise : undefined,
      category: "",
      level: "",
    });
  }
...
export const calculateExamScore = (
  ...
  scorableQuestions.forEach((q: Question) => {
    ...
    if (
      ...
      if (
        q.correctAnswer &&
        isAnswerMatch(q.correctAnswer, studentAnswer, q.questionType)
      )
        isCorrect = true;
    } else if (q.questionType === "COMPLEX_MULTIPLE_CHOICE") {
      const studentSet = new Set(
        parseList(studentAnswer).map((a) => normalize(a, q.questionType)),
      );
...
export const sanitizeHtml = (html: string): string => {
  ...
  const isAksaraBali = (el: Element) =>
    ...
    (el instanceof HTMLElement &&
      el.style.fontFamily.includes("Noto Sans Balinese"));
...
export const htmlToMarkdown = (html: string): string => {
  ...
  // To: $\frac{1}{2}$
  const mathRegex =
    /(?:&#8203;|\u200B)?<span[^>]*class="math-visual"[^>]*data-latex="([^"]*)"[^>]*>.*?<\/span>(?:&#8203;|\u200B)?/g;
...
export const markdownToHtml = (markdown: string): string => {
  ...
  audios.forEach((src, i) => {
    html = html.replace(
      `%%%AUDIO_${i}%%%`,
      `<br/><audio controls src="${src}" style="max-width: 100%; display: block; margin: 8px 0;"></audio><br/>`,
    );
  });
...
export const cleanupQuestionContent = (q: any): any => {
  ...
  if (qClean.trueFalseRows && Array.isArray(qClean.trueFalseRows)) {
    qClean.trueFalseRows = qClean.trueFalseRows.map((row: any) => ({
      ...row,
      text: minifyExamHtml(row.text),
    }));
  }
  ...
  try {
    ...
    const injectChart = (chartData: ChartData | undefined | null, html: string): string => {
      ...
      if (chartData) {
        ...
        if (typedData.type === "venn") {
          ...
          const originY =
            h - padding - (Math.abs(config.yMin) / yRange) * graphH;
          ...
          let gridHtml = "";
          xTicks.forEach((x) => {
            gridHtml += `<line x1="${mapX(x)}" y1="${padding}" x2="${mapX(x)}" y2="${h - padding}" stroke="#f1f5f9" stroke-width="1" />`;
          });
          yTicks.forEach((y) => {
            gridHtml += `<line x1="${padding}" y1="${mapY(y)}" x2="${w - padding}" y2="${mapY(y)}" stroke="#f1f5f9" stroke-width="1" />`;
          });
          ...
          const datasetsStr = JSON.stringify(
            typedData.datasets?.map((ds: any, idx: number) => {
              const colors = [
                "#0088FE",
                "#00C49F",
                "#FFBB28",
              ...
              return {
                ...
                backgroundColor:
                  ds.backgroundColor ||
                  (typedData.type === "pie" ? colors : color),
      ...
      if (
        ...
        if (q.matchingPairs) {
          ...
          q.matchingPairs.forEach((pair) => {
            ...
            if (pair.leftChart)
              leftHtml = injectChart(pair.leftChart, leftHtml);
    ...
    scorableQuestions.forEach((q, index) => {
      ...
      if (q.questionType === "COMPLEX_MULTIPLE_CHOICE") {
        ansStr = parseList(q.correctAnswer || "")
          .map((s) => String(s))
          .join(", ");
      } else if (q.questionType === "TRUE_FALSE") {
        if (q.trueFalseRows) {
        ...
        if (q.matchingPairs) {
          ansStr = q.matchingPairs
            .map((p) => `${p.left} &rarr; ${p.right}`)
            .join("<br/>");
        }
