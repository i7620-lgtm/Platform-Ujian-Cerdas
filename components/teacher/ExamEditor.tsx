...
const WysiwygEditor: React.FC<{ 
    ...
    const insertMath = (latex: string) => { if ((window as any).katex) { const html = (window as any).katex.renderToString(latex, { throwOnError: false }); const wrapper = `<span class="math-visual inline px-0.5 rounded select-none cursor-pointer hover:bg-indigo-50 dark:hover:bg-slate-700 align-middle" contenteditable="false" data-latex="${latex.replace(/"/g, '&quot;')}">${html}</span><span style="font-size: 100%; font-family: inherit; font-weight: normal; font-style: normal; color: inherit;">&nbsp;</span>`; runCmd('insertHTML', wrapper); handleInput(); } };
