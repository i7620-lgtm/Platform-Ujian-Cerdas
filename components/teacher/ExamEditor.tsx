...
const WysiwygEditor: React.FC<{ value: string; onChange: (val: string) => void; placeholder?: string; minHeight?: string; showTabs?: boolean }> = ({ value, onChange, placeholder = "Ketik di sini...", minHeight = "120px", showTabs = true }) => {
    ...
    const insertTable = (rows: number, cols: number) => { let html = '<table class="border-collapse border border-slate-300 dark:border-slate-600 my-2 w-full text-sm"><thead><tr>'; for(let c=0; c<cols; c++) html += `<th class="border border-slate-300 dark:border-slate-600 p-2 bg-slate-50 dark:bg-slate-800">H${c+1}</th>`; html += '</tr></thead><tbody>'; for(let r=0; r<rows; r++) { html += '<tr>'; for(let c=0; c<cols; c++) html += `<td class="border border-slate-300 dark:border-slate-600 p-2">Data</td>`; html += '</tr>'; } html += '</tbody></table><p><br/></p>'; runCmd('insertHTML', html); handleInput(); };
    ...
    const editorStyle = `.wysiwyg-content table { width: 100%; border-collapse: collapse; margin: 0.5rem 0; } .wysiwyg-content th, .wysiwyg-content td { border: 1px solid #cbd5e1; padding: 0.5rem; min-width: 30px; } .dark .wysiwyg-content th, .dark .wysiwyg-content td { border-color: #475569; } .wysiwyg-content th { background-color: #f8fafc; font-weight: bold; text-align: left; } .dark .wysiwyg-content th { background-color: #1e293b; color: #e2e8f0; } .wysiwyg-content:empty:before { content: attr(data-placeholder); color: #94a3b8; font-style: italic; } .dark .wysiwyg-content:empty:before { color: #64748b; } .wysiwyg-content ul { list-style-type: disc; padding-left: 1.5rem; } .wysiwyg-content ol { list-style-type: decimal; padding-left: 1.5rem; } .wysiwyg-content blockquote { border-left: 3px solid #cbd5e1; padding-left: 1rem; color: #64748b; font-style: italic; } .dark .wysiwyg-content blockquote { border-color: #475569; color: #94a3b8; } .dark .wysiwyg-content { color: #f1f5f9; }`;
