...
const WysiwygEditor: React.FC<{ value: string; onChange: (val: string) => void; placeholder?: string; minHeight?: string; showTabs?: boolean }> = ({ value, onChange, placeholder = "Ketik di sini...", minHeight = "120px", showTabs = true }) => {
    ...
    // Updated Editor Style for Better Dark Mode Support on Tables
    const editorStyle = `
        .wysiwyg-content table { width: 100%; border-collapse: collapse; margin: 0.5rem 0; } 
        .wysiwyg-content th, .wysiwyg-content td { border: 1px solid #cbd5e1; padding: 0.5rem; min-width: 30px; color: inherit; } 
        .dark .wysiwyg-content th, .dark .wysiwyg-content td { border-color: #334155; } 
        .wysiwyg-content th { background-color: #f8fafc; font-weight: bold; text-align: left; } 
        .dark .wysiwyg-content th { background-color: #1e293b; color: #e2e8f0; } 
        .dark .wysiwyg-content td { background-color: transparent; color: #f1f5f9; }
