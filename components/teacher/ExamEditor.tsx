...
// ... (Other helper modals like TableConfigModal, VisualMathModal follow similar pattern of dark mode classes: bg-white -> dark:bg-slate-800, text-slate-800 -> dark:text-white, etc.) ...
...
export const ExamEditor: React.FC<ExamEditorProps> = ({ 
    ...
    // Note: Ensure all inputs below have dark:bg-slate-900 dark:text-slate-200 dark:border-slate-700 classes.
    ...
    return (
        <div className="space-y-10 border-t-2 border-gray-200 dark:border-slate-700 pt-12">
                                ...
                                <div id={q.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 group transition-all duration-300 hover:shadow-md relative overflow-visible">
                                     <div className="absolute top-4 right-4 flex gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity z-20">
                                         <div className="relative inline-block bg-white dark:bg-slate-900 rounded-lg shadow-sm">
                                            ...
                                            <select value={q.questionType} onChange={(e) => { /* handleTypeChange */ }} className="appearance-none bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-200 py-1.5 pl-3 pr-7 rounded-lg text-[10px] font-bold uppercase tracking-wider cursor-pointer hover:bg-white dark:hover:bg-slate-600 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all">
                                    ...
                                    <div className="p-6 md:p-8">
                                        <div className="flex items-start gap-4 md:gap-6">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <WysiwygEditor value={q.questionText} onChange={(val) => { /* handleQuestionTextChange */ }} placeholder={q.questionType === 'INFO' ? "Tulis informasi atau teks bacaan di sini..." : "Tulis pertanyaan di sini..."} minHeight="80px" />
                                                
                                                {/* OPTION RENDERERS WITH DARK MODE */}
                                                {q.questionType === 'MULTIPLE_CHOICE' && q.options && (
                                                    <div className="mt-6 space-y-3">
                                                        {q.options.map((option, i) => (
                                                            <div key={i} className={`group/opt relative flex items-start p-1 rounded-xl transition-all ${q.correctAnswer === option ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : ''}`}>
                                                                <div className="flex items-center h-full pt-4 pl-2 pr-4 cursor-pointer" onClick={() => { /* handleCorrectAnswer */ }}><div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${q.correctAnswer === option ? 'border-emerald-500 bg-emerald-500' : 'border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 group-hover/opt:border-emerald-300'}`}>{q.correctAnswer === option && <div className="w-2 h-2 bg-white rounded-full" />}</div></div>
                                                                <div className="flex-1"><WysiwygEditor value={option} onChange={(val) => { /* handleOptionTextChange */ }} placeholder={`Opsi ${String.fromCharCode(65 + i)}`} minHeight="40px" /></div>
                                                                <div className="flex flex-col gap-1 opacity-0 group-hover/opt:opacity-100 transition-opacity px-2 pt-2"><button type="button" onClick={(e) => { e.stopPropagation(); /* handleDeleteOption */ }} className="text-gray-300 dark:text-slate-600 hover:text-red-500"><TrashIcon className="w-4 h-4"/></button></div>
                                                            </div>
                                                        ))}
                                                        <button onClick={() => { /* handleAddOption */ }} className="ml-12 mt-2 text-xs font-bold text-primary hover:text-primary-focus flex items-center gap-1 opacity-60 hover:opacity-100"><PlusCircleIcon className="w-4 h-4" /> Tambah Opsi</button>
                                                    </div>
                                                )}
                                                {/* ... (Other types similar updates) ... */}
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            {/* Modals placed here but using state controls above */}
            <SelectionModal isOpen={isSubjectModalOpen} title="Pilih Mata Pelajaran" options={SUBJECTS} selectedValue={config.subject || ''} onClose={() => setIsSubjectModalOpen(false)} onSelect={(val) => { /* setConfig */ }} searchPlaceholder="Cari mata pelajaran..." />
            {/* ... other modals ... */}
        </div>
    );
};
