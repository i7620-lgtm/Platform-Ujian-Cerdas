                        ...
                        {questionAnalysisData.map((data, idx) => (
                            <div key={data.id} className="avoid-break border border-slate-300 rounded p-3 text-xs flex flex-col gap-2 bg-white">
                                <div>
                                    ...
                                    <div className="text-[10px] text-slate-500 line-clamp-2 italic mb-1 border-b border-slate-100 pb-1" dangerouslySetInnerHTML={{ __html: data.qText }}></div>
                                ...
                                <div className="pt-1">
                                    ...
                                    {/* Updated Distribution Rendering for All Types */}
                                    {data.options ? (
                                        ...
                                        <div className="grid grid-cols-2 gap-1 text-[9px]">
                                            {data.options.map((opt, i) => {
                                                ...
                                                return (
                                                    <div key={i} className={`flex items-center justify-between px-1.5 py-0.5 rounded border ${isCorrect ? 'bg-emerald-50 border-emerald-200 text-emerald-800 font-bold' : 'border-slate-100 text-slate-600'}`}>
                                                        <span>{label}</span>
                                        ...
                                        <div className="flex flex-wrap gap-1 text-[9px]">
                                            {Object.entries(data.distribution).length > 0 ? (
                                                Object.entries(data.distribution)
                                                    ...
                                                    .slice(0, 6) // Limit to top 6 answers to save space
                                                    .map(([ans, count], i) => {
                                                        const pct = totalStudents > 0 ? Math.round((count/totalStudents)*100) : 0;
                                                        // Attempt to detect correct answer for styling
                                                        const qObj = exam.questions.find(q=>q.id===data.id);
                                                        let isCorrect = false;
                                                        try {
                                                            const normAns = normalize(ans);
                                                            const normKey = normalize(qObj?.correctAnswer || '');
                                                            if (qObj?.questionType === 'TRUE_FALSE') {
                                                                 // True False usually stored as JSON, harder to match directly here without parsing, but often distribution key is raw text answer if parsing failed in analysis hook, or stringified json. 
                                                                 // For simplicity in print, just showing counts is often enough, but let's try strict match
                                                        ...
                                                        return (
                                                            <div key={i} className={`flex items-center gap-1 px-1.5 py-0.5 rounded border ${isCorrect ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                                                                <span className="truncate max-w-[80px]" title={ans}>{ans}</span>
                                            ...
                                            {Object.keys(data.distribution).length > 6 && <span className="text-slate-400 px-1">...</span>}
                                        </div>
                                    )}
                                </div>
                            </div>
