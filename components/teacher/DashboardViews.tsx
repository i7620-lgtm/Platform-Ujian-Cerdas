...
export const FinishedExamsView: React.FC<FinishedExamsProps> = ({ exams, results, onSelectExam, onDuplicateExam, onDeleteExam, teacherProfile }) => {
    const [archivingId, setArchivingId] = useState<string | null>(null);

    const handleArchive = async (exam: Exam) => {
        setArchivingId(exam.code);
        try {
            // 1. Coba filter dari props (memori aplikasi)
            let examResults = results.filter(r => r.examCode === exam.code);
            
            // 2. Jika kosong (mungkin belum tersinkron), fetch data terbaru dari server
            if (examResults.length === 0) {
                ...
                const fetched = await storageService.getResults(exam.code, undefined, headers);
                if (fetched && fetched.length > 0) {
                    examResults = fetched;
                }
            }

            // 3. Filter status (ambil semua kecuali in_progress untuk arsip final)
            // Anda bisa menghapus filter ini jika ingin in_progress juga masuk
            const finalResults = examResults.filter(r => r.status !== 'in_progress');
            ...
            alert("Gagal mengunduh arsip. Silakan coba lagi.");
            console.error(e);
        } finally {
            setArchivingId(null);
        }
                        ...
                        <div key={exam.code} className="bg-white p-5 rounded-2xl border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all hover:shadow-md hover:border-gray-300 group relative">
                            ...
                            <div className="flex items-center gap-2 self-end md:self-center w-full md:w-auto">
                                <button 
                                    ...
                                    disabled={archivingId === exam.code}
                                    className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 px-4 py-2.5 text-sm rounded-xl hover:bg-indigo-100 hover:text-indigo-800 transition-colors font-bold shadow-sm border border-indigo-100 disabled:opacity-50 disabled:cursor-not-allowed"
                                    title="Download Arsip JSON"
                                >
                                    {archivingId === exam.code ? (
                                        <div className="w-4 h-4 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
                                    ) : (
                                        <ArrowDownTrayIcon className="w-4 h-4" />
                                    )}
                                    <span className="hidden lg:inline">{archivingId === exam.code ? 'Memuat...' : 'Arsip'}</span>
                                </button>
                                <button 
