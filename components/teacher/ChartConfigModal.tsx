...
export const ChartConfigModal: React.FC<ChartConfigModalProps> = ({
  ...
  const {
    ...
    handleDeleteDataset,
    handleSave,
  } = useChartConfigModal({ isOpen, initialData, onClose, onSave });
          ...
          {type === "relation" ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newL = [...labels];
                      newL[0] = e.target.value;
                      setLabels(newL);
                    }}
                    className="w-full p-2 border rounded focus:ring-1 outline-none dark:bg-slate-800 dark:border-slate-700 dark:text-white font-bold"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">
                    Nama Himpunan 2 (Kodomain)
                  </label>
                  <input
                    type="text"
                    value={labels[1] || ""}
                    ...
                    {datasets[0]?.data.map((item, idx) => (
                      <div key={`dom-${idx}`} className="flex gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            const newD = [...datasets];
                            newD[0].data[idx] = e.target.value;
                            setDatasets(newD);
                          }}
                    ...
                    {datasets[1]?.data.map((item, idx) => (
                      <div key={`codom-${idx}`} className="flex gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            const newD = [...datasets];
                            newD[1].data[idx] = e.target.value;
                            setDatasets(newD);
                          }}
                          className="flex-1 p-1 text-sm border rounded dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                        />
                ...
                <div className="grid grid-cols-1 gap-2">
                  {datasets[2]?.data.map((rel, idx) => {
                    ...
                    return (
                      <div
                        key={`rel-${idx}`}
                        className="flex gap-2 items-center bg-white dark:bg-slate-800 p-2 rounded border dark:border-slate-700"
                      >
                        <select
                          value={dIdx}
                          onChange={(e) => {
                            const newD = [...datasets];
                            newD[2].data[idx] = `${e.target.value}-${cIdx}`;
                            setDatasets(newD);
                          }}
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const newD = [...datasets];
                            newD[2].data[idx] = `${dIdx}-${e.target.value}`;
                            setDatasets(newD);
                          }}
                          className="flex-1 p-1 text-sm border rounded dark:bg-slate-700 dark:border-slate-600 dark:text-white"
                        >
            ...
            <div className="space-y-4 bg-gray-50 dark:bg-slate-800/30 p-4 rounded-xl border dark:border-slate-700">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-gray-700 dark:text-slate-200">
                  Data Diagram Venn
                </h3>
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 py-1 rounded text-sm ${labels.length !== 3 ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-slate-700 dark:text-slate-300"}`}
                  >
                    2 Himpunan
                  </button>
                  <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={(datasets[0]?.data[labels.length === 3 ? 8 : 4] as string | number) || ""}
                    onChange={(e) => {
                      const newD = [...(datasets[0]?.data || [])];
                      newD[labels.length === 3 ? 8 : 4] = e.target.value;
                      const newDatasets = [...datasets];
                      if (!newDatasets[0])
                        newDatasets[0] = { label: "Data", data: [] };
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">
                    Nilai di Luar Himpunan
                  </label>
                  <input
                    type="text"
                    value={(datasets[0]?.data[labels.length === 3 ? 7 : 3] as string | number) || ""}
                    onChange={(e) => {
                      const newD = [...(datasets[0]?.data || [])];
              ...
              <div className="space-y-3 mt-4">
                <h4 className="font-bold text-sm text-slate-600 dark:text-slate-300 border-b dark:border-slate-700 pb-1">
                  Himpunan
                </h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    ...
                    <label className="text-xs text-slate-500">
                      Nilai Hanya {labels[0] || "A"}
                    </label>
                    <input
                      type="text"
                      value={(datasets[0]?.data[0] as string | number) ?? ""}
                      onChange={(e) => {
                        const newD = [...(datasets[0]?.data || [])];
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    />
                    <label className="text-xs text-slate-500">
                      Nilai Hanya {labels[1] || "B"}
                    </label>
                    <input
                      type="text"
                      value={(datasets[0]?.data[1] as string | number) ?? ""}
                      onChange={(e) => {
                        const newD = [...(datasets[0]?.data || [])];
                        newD[1] = e.target.value;
                        const newDatasets = [...datasets];
                        if (!newDatasets[0])
                          newDatasets[0] = { label: "Data", data: [] };
                        newDatasets[0].data = newD;
                  ...
                  {labels.length === 3 && (
                    <div className="flex flex-col gap-1 col-span-2 sm:col-span-1">
                      <input
                        type="text"
                        placeholder="Nama Himpunan 3"
                        value={labels[2] || "C"}
                        onChange={(e) => {
                          const newL = [...labels];
                          newL[2] = e.target.value;
                          setLabels(newL);
                        }}
                        className="w-full p-2 text-sm border rounded focus:ring-1 outline-none dark:bg-slate-800 dark:border-slate-700 dark:text-white font-bold"
                      />
                      <label className="text-xs text-slate-500">
                        Nilai Hanya {labels[2] || "C"}
                      </label>
                      <input
                        type="text"
                        value={(datasets[0]?.data[2] as string | number) ?? ""}
                        onChange={(e) => {
                          const newD = [...(datasets[0]?.data || [])];
                          newD[2] = e.target.value;
                          const newDatasets = [...datasets];
                          if (!newDatasets[0])
                            newDatasets[0] = { label: "Data", data: [] };
                          newDatasets[0].data = newD;
              ...
              <div className="space-y-3 mt-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-xs text-slate-500">
                      Irisan {labels[0] || "A"} & {labels[1] || "B"}
                    </label>
                    <input
                      type="text"
                      value={
                        (datasets[0]?.data[labels.length === 3 ? 3 : 2] as string | number) ?? ""
                      }
                      onChange={(e) => {
                        const newD = [...(datasets[0]?.data || [])];
                        newD[labels.length === 3 ? 3 : 2] = e.target.value;
                        const newDatasets = [...datasets];
                        if (!newDatasets[0])
                          newDatasets[0] = { label: "Data", data: [] };
                        newDatasets[0].data = newD;
                  ...
                  {labels.length === 3 && (
                    <>
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-slate-500">
                          Irisan {labels[0] || "A"} & {labels[2] || "C"}
                        </label>
                        <input
                          type="text"
                          value={(datasets[0]?.data[4] as string | number) ?? ""}
                          onChange={(e) => {
                            const newD = [...(datasets[0]?.data || [])];
                            newD[4] = e.target.value;
                            const newDatasets = [...datasets];
                            if (!newDatasets[0])
                              newDatasets[0] = { label: "Data", data: [] };
                            newDatasets[0].data = newD;
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-xs text-slate-500">
                          Irisan {labels[1] || "B"} & {labels[2] || "C"}
                        </label>
                        <input
                          type="text"
                          value={(datasets[0]?.data[5] as string | number) ?? ""}
                          onChange={(e) => {
                            const newD = [...(datasets[0]?.data || [])];
                            newD[5] = e.target.value;
                            const newDatasets = [...datasets];
                            if (!newDatasets[0])
                              newDatasets[0] = { label: "Data", data: [] };
                            newDatasets[0].data = newD;
                      ...
                      <div className="flex flex-col gap-1 col-span-2">
                        <label className="text-xs text-slate-500 font-bold">
                          Irisan Ketiganya ({labels[0] || "A"},{" "}
                          {labels[1] || "B"} & {labels[2] || "C"})
                        </label>
                        <input
                          type="text"
                          value={(datasets[0]?.data[6] as string | number) ?? ""}
                          onChange={(e) => {
                            const newD = [...(datasets[0]?.data || [])];
                            newD[6] = e.target.value;
                            const newDatasets = [...datasets];
                            if (!newDatasets[0])
                              newDatasets[0] = { label: "Data", data: [] };
                            newDatasets[0].data = newD;
              ...
              <div className="overflow-x-auto border dark:border-slate-700 rounded-xl">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 dark:bg-slate-800/50 border-b dark:border-slate-700">
                    <tr>
                      <th className="p-3 text-left font-medium text-gray-600 dark:text-slate-300">
                        Label
                      </th>
                      {datasets.map((d, i) => (
                        <th
                          key={i}
                          className="p-3 text-left font-medium text-gray-600 dark:text-slate-300"
                        >
                          <div className="flex items-center gap-2">
                            <input
                              type="text"
                      ...
                      >
                        ...
                        {datasets.map((dataset, datasetIdx) => (
                          <td key={datasetIdx} className="p-3">
                            <input
                              type="number"
                              value={(dataset.data[labelIdx] as string | number) ?? ""}
                              onChange={(e) => {
                                const newDatasets = [...datasets];
                                newDatasets[datasetIdx].data[labelIdx] = Number(
                                  e.target.value,
                                );
                                setDatasets(newDatasets);
                              }}
                        ...
                        <td className="p-3">
                          {labels.length > 1 && (
                            <button
                              onClick={() => handleDeleteLabel(labelIdx)}
                              className="text-red-500 hover:text-red-700"
                            >
                              <TrashIcon className="w-4 h-4" />
                            </button>
