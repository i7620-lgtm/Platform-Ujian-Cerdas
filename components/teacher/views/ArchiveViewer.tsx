                                ...
                                <div key={q.id} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 space-y-4 min-w-0">
                                            ...
                                            {q.questionType === 'MATCHING' && q.matchingPairs && <div className="space-y-2">{q.matchingPairs.map((p,i) => <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 text-sm"><div className="flex-1 min-w-0 font-medium dark:text-slate-200 option-content" dangerouslySetInnerHTML={{ __html: p.left }}></div><div className="text-slate-300 dark:text-slate-500">→</div><div className="flex-1 min-w-0 font-bold dark:text-slate-200 option-content" dangerouslySetInnerHTML={{ __html: p.right }}></div></div>)}</div>}
                                        ...
                                        <div className="flex-1 space-y-3 min-w-0">
                                            ...
                                            {q.questionType === 'MULTIPLE_CHOICE' && q.options && q.options.map((opt, i) => (
                                                <div key={i} className={`flex items-start gap-3 p-2 rounded-lg border text-xs ${isAnswerMatch(q.correctAnswer, opt, q.questionType) ? 'bg-emerald-50 border-emerald-200 font-bold text-emerald-800 print-bg-green' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                                                    ...
                                                    <div className="flex-1 min-w-0 option-content" dangerouslySetInnerHTML={{ __html: opt }}></div>
                                                ...
                                                <div className="space-y-1">
                                                    {q.matchingPairs.map((p,i) => (
                                                        <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-slate-50 border border-slate-200 text-xs">
                                                            <div className="flex-1 min-w-0 font-medium option-content" dangerouslySetInnerHTML={{ __html: p.left }}></div>
                                                            <div className="text-slate-400">→</div>
                                                            <div className="flex-1 min-w-0 font-bold text-emerald-700 option-content" dangerouslySetInnerHTML={{ __html: p.right }}></div>
                                                        </div>
                                                    ))}
                                                </div>
                                            )}
