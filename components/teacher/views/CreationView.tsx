...
export const CreationView: React.FC<CreationViewProps> = ({ onQuestionsGenerated }) => {
    ...
    const handleExtractText = async () => { if (!uploadedFile) return; setIsLoading(true); try { const text = await extractTextFromPdf(uploadedFile); setInputText(text); setInputMethod('paste'); } catch { setError("Gagal mengekstrak teks dari PDF."); } finally { setIsLoading(false); } };
