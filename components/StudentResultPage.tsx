...
export const StudentResultPage: React.FC<StudentResultPageProps> = ({ result, exam, onFinish }) => {
    ...
    const handleUnlockSubmit = async (e: React.FormEvent) => {
        ...
        if (!unlockToken || unlockToken.length < 4) {
            setUnlockError("Token minimal 4 digit.");
            return;
        }
        setIsUnlocking(true);
        setUnlockError('');
        ...
        try {
            const success = await storageService.verifyUnlockToken(exam.code, result.student.studentId, unlockToken);
            if (success) {
                // Reload page to re-initialize session with new 'in_progress' status
                ...
                <div className="w-full max-w-sm text-center bg-white p-8 rounded-3xl shadow-xl border border-rose-100 animate-fade-in">
                    ...
                    <form onSubmit={handleUnlockSubmit} className="mb-6 space-y-3">
                        <input 
                            ...
                            className="w-full text-center text-xl font-mono font-bold tracking-[0.2em] py-3 bg-slate-50 border-2 border-slate-200 rounded-xl focus:border-rose-400 focus:bg-white outline-none transition-all uppercase"
                            placeholder="TOKEN"
                            maxLength={6}
                        />
                        ...
                        <button 
                            ...
                            className="w-full bg-rose-500 text-white font-bold py-3 rounded-xl hover:bg-rose-600 transition-all text-sm shadow-lg shadow-rose-200 disabled:opacity-70"
                        >
                            {isUnlocking ? 'Memproses...' : 'Buka Kunci'}
                        </button>
                    </form>
