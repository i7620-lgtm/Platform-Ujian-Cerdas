...
export const StudentResultPage: React.FC<StudentResultPageProps> = ({ result, exam, onFinish, onResume, isDarkMode, toggleTheme }) => {
    ...
    const handleDownloadCertificate = async () => {
        ...
        try {
            ...
            const displayScore = result.status === 'completed' || result.status === 'force_closed' 
                ? (result.score === 0 && hasAutoGradable && (calculatedStats?.calculatedScore || 0) > 0 ? calculatedStats?.calculatedScore || 0 : result.score) 
                : 0;

            const qrLink = `${window.location.origin}/result/${encodeURIComponent(exam.code)}/${encodeURIComponent(result.student.studentId)}`;
            const verifyCode = result.id ? result.id.toString(16).toUpperCase().padStart(5, '0') : '0X98A';
