...
export const CollaboratorView: React.FC<CollaboratorViewProps> = ({
  exam,
  role,
  token,
  onExit,
  ...
  const handleSave = async () => {
    try {
      if (token) {
        ...
        await storageService.saveCollaboratorExam(
          { ...exam, questions: sanitizedQuestions, config },
          token,
        );
      } else {
      ...
      console.error(e);
      const errorMessage =
        e instanceof Error ? e.message : "Gagal menyimpan perubahan.";
      alert(errorMessage);
    }
  };
      ...
      <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 sticky top-0 z-40">
        <div className="w-full max-w-full mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={onExit}
              className="flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-rose-600 dark:text-slate-400 dark:hover:text-rose-400 uppercase tracking-wider transition-colors"
            >
