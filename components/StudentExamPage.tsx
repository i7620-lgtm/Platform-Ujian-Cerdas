...
export const StudentExamPage: React.FC<StudentExamPageProps> = ({ exam, student, initialData, onSubmit, isDarkMode, toggleTheme }) => {
    ...
    const handleSubmit = useCallback(async (isAuto = false, status: ResultStatus = 'completed') => {
        ...
        if (!isAuto) {
            ...
            const unansweredCount = activeExam.questions.filter(q => q.questionType !== 'INFO' && !isAnswered(q, answersRef.current)).length;
                                                    ...
                                                    {q.options.map((opt, i) => {
                                                        ...
                                                        return (
                                                            <button key={i} onClick={() => { 
                                                                ...
                                                                <div className={`w-6 h-6 rounded-md flex items-center justify-center border text-xs font-black mt-0.5 shrink-0 transition-colors ${isSelected ? 'bg-indigo-600 dark:bg-indigo-500 border-indigo-600 dark:border-indigo-500 text-white shadow-sm' : 'border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800'}`}>{String.fromCharCode(65 + i)}</div>
                                                                <div className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-medium option-content flex-1 min-w-0" dangerouslySetInnerHTML={{ __html: optimizeHtml(opt) }}></div>
                                                            </button>
                                                        );
                                                    })}
