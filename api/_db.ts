...
// --- SMART KEY FORMATTER (ULTRA-ROBUST) ---
const formatPrivateKey = (key: string | undefined): string | null => {
    if (!key) return null;
    
    // 1. Handle escaped newlines and remove surrounding quotes
    let cleanKey = key.replace(/^["']|["']$/g, '').split(String.raw`\n`).join('\n');
    
    const header = "-----BEGIN PRIVATE KEY-----";
    const footer = "-----END PRIVATE KEY-----";

    // 2. Extract body by removing header/footer, then remove all whitespace from body
    const body = cleanKey
        .replace(header, '')
        .replace(footer, '')
        .replace(/\s/g, ''); // Removes spaces, newlines, tabs etc. from the key body

    // 3. Reconstruct the key in a valid PEM format. Node's crypto lib is fine with a single-line body.
    return `${header}\n${body}\n${footer}`;
};
